...
- **配置**（`backend/.env`）：
  ...
  ```
  质量规范独立于安全规范（`data_demo/standards/quality_standards.json`，与 safety 一样收录真实国标条款——GB 50204-2015《混凝土结构工程施工质量验收规范》、GB 50207-2012《屋面工程质量验收规范》、GB 50210-2018《建筑装饰装修工程质量验收标准》、GB 50205-2020《钢结构工程施工质量验收标准》等，条号可溯源）；RagAgent 按 `AgentRun.module` 检索对应知识库；
- **接口**：
  | 接口 | 说明 |
  | --- | --- |
...
「工友关怀」页（`/worker-wellbeing`）为工友幸福服务：输入天气/环境数据（温度、湿度、天气现象），**CareAgent** 计算高温等级、中暑风险指数与体感温度，输出温馨、可执行的关怀提醒、中暑急救知识与现场福利设施信息；内置浏览器噪声监测（麦克风实时测 dB，70dB 以上加强提醒、85dB 以上必须听力防护、90dB 以上建议暂停高噪声作业并复测）；红色高温（≥40℃）自动联动网络音响/PA 语音广播与现场蜂鸣器。
...
「工友助手」页（`/worker-care`）把专业整改要求转成尊重、简短、可执行的现场提醒，回答由规范知识库 RAG 检索生成（正文保持简短平和、不内嵌来源；命中的规范条款单独以「来源」标签展示在回答下方，高风险项提示暂停作业），未命中时回退本地模板，不替代安全员判断。
...
| 账号 | 密码 | 角色 | 登录默认落地页 |
| --- | --- | --- | --- |
| `admin` | `BuildWise123!` | 管理员 | 项目工作台 |
| `manager` | `BuildWise123!` | 项目经理 | 项目工作台 |
| `safety` | `BuildWise123!` | 安全员 | 现场安全分析 |
| `quality` | `BuildWise123!` | 质检员 | 质量巡检 |
| `worker` | `BuildWise123!` | 工人 | 工友助手 |

四个演示账号登录后功能不同，前端按角色过滤菜单与路由，后端接口同步 `require_roles` 门控（越权返回 403，双层一致）：

| 功能 | manager | safety | quality | worker |
| --- | --- | --- | --- | --- |
| 项目工作台 | ✓ | ✓ | ✓ | ✓ |
| 项目管理 | ✓ | — | — | — |
| 现场安全分析 | ✓ | ✓ | — | — |
...
- `admin` 拥有全部功能（含权限审计）；
- 整改工单：全员可查看，仅 admin / manager / safety / quality 可确认创建（worker 只能查看，创建返回 403）；
- 规范知识库：全员可检索问答，新增文档 / 重建索引限 admin / manager / safety / quality；
- 项目管理页限 admin / manager，但 worker 工作台仍可通过开放接口读取项目列表；
