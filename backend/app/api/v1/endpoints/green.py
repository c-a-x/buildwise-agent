...
def create_assessment(form: GreenAssessmentForm, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_project_access(form.project_id, user, db)
...
def list_assessments(http_request: Request, project_id: str | None = Query(None), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if project_id:
        ensure_project_access(project_id, user, db)
...
def get_assessment(assessment_id: str, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_assessment_report(assessment_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def env_thresholds(http_request: Request, user: User = Depends(get_current_user)):
    return ok([item.model_dump(mode="json") for item in GreenEnvService.thresholds()], http_request)


@router.post("/env-records")
def upsert_env_record(form: GreenEnvRecordForm, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_project_access(form.project_id, user, db)
...
def get_env_record(record_id: str, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.get("/trend")
def trend(http_request: Request, project_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_project_access(project_id, user, db)
...
@router.get("/target")
def get_target(http_request: Request, project_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_project_access(project_id, user, db)
...
@router.put("/target")
def set_target(form: GreenTargetForm, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_project_access(form.project_id, user, db)
...
@router.post("/advice")
def generate_advice(form: GreenAdviceForm, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_project_access(form.project_id, user, db)
