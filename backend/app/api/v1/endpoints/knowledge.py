...
from app.api.dependencies import ensure_project_access, get_current_user, require_roles
...
_KNOWLEDGE_WRITE_ROLES = ("admin", "project_manager", "safety_officer", "quality_inspector")
...
def create(request: KnowledgeDocumentCreate, http_request: Request, user: User = Depends(require_roles(*_KNOWLEDGE_WRITE_ROLES)), db: Session = Depends(get_db)):
...
@router.post("/reindex")
def reindex(http_request: Request, user: User = Depends(require_roles(*_KNOWLEDGE_WRITE_ROLES)), db: Session = Depends(get_db)):
