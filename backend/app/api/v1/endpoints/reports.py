...
def generate(request: DailyReportGenerate, http_request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.get("/daily")
def daily(http_request: Request, project_id: str, report_date: date = Query(...), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.get("")
def history(http_request: Request, project_id: str | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
