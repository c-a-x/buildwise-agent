...
class OpenAICompatibleTextProvider:
    ...
    def generate_worker_answer(self, payload: dict[str, object]) -> str:
        ...
        prompt = (
            "你是一名工地安全工友助手，用通俗易懂、尊重、平和的普通话回答工人的问题。\n"
            f"工人问题：{question}\n\n"
            "可依据的规范条款如下（只能引用这些条款的内容，不得编造任何规范、条款或数字）：\n"
            f"{clauses_text}\n\n"
            "要求：\n"
            "1. 直接回答问题，给出具体、可执行的安全建议；\n"
            "2. 回答正文中不要提及任何《来源》或第X条等条款名称，引用由页面单独展示；若上方条款为空（知识库暂未检索到条款），请明确说明该回答为一般性安全建议、并非知识库条款，不要编造条款、规范或数字；\n"
            "3. 不要假设现场已发生违规或事故，不要凭空要求「暂停作业」；\n"
            "4. 语气尊重、平和，说工人听得懂的大白话，但避免过于随意的口头语；\n"
            "5. 控制在 150 字以内。"
        )
        return self._generate(prompt)
