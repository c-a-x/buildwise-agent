...
function announceResult(): void {
  ...
  const current = quality.currentResult
  if (!current) return
  speakHazards(current.defects.filter(isAnnounceableHazard).map((defect) => defect.hazard_name))
}
