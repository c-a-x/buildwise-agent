...
async function loadHistoryTask(): Promise<void> {
  ...
  if (!taskId) return
  localError.value = ''
  try { await safety.loadTask(taskId) } catch (cause) { localError.value = getApiError(cause) }
}
...
.broadcast-toggle.on { border-color: var(--primary); color: var(--surface); background: var(--primary); }
