...
async function load(): Promise<void> {
  ...
  loading.value = true
  error.value = ''
  try { orders.value = await workOrdersApi.list({ project_id: projects.currentProject.id, status: status.value || undefined, risk_level: risk.value || undefined }) } catch (cause) { error.value = getApiError(cause) } finally { loading.value = false }
}
